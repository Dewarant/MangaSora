...
class BookForm(FlaskForm):
    ...
    cover = FileField('Обложка манги', validators=[FileAllowed(['jpg', 'png'])])
    ...
    price = IntegerField('Цена', validators=[DataRequired(), NumberRange(min=1, max=50000)])
    submit = SubmitField('Добавить')

    def validate_title(self, title):
        title = Book.query.filter_by(title=title.data).first()
        if title:
            raise ValidationError('Такая манга уже есть.')
